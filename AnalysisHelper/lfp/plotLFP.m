function plotLFP(LFPinfo, exinfo, stmtype, ps_flag)
%% plot LFP data across sessions
%
%
% written by Katsuhisa (02.10.17)
% ++++++++++++++++++++++++++++++++
...
if ps_flag == 0
    ...
    % LFP
    h = figure;
    len_trace = length(LFPinfo.session(row(1)).results.cond(1).stlfp.mean);
    len_pow = length(LFPinfo.session(row(1)).results.cond(1).stlfp.pow);
    for k = 1:2
    ...
    for i = 1:lenr
        for k = 1:2
            para.cond(k).stlfp.zeroval(i) = ...
                LFPinfo.session(row(i)).results.cond(k).stlfp.mean(round(len_trace/2));
            para.cond(k).stlfp.trace(i,:) = ...
                LFPinfo.session(row(i)).results.cond(k).stlfp.mean;
            para.cond(k).stlfp.pow(i,:) = ...
                LFPinfo.session(row(i)).results.cond(k).stlfp.pow';
        end
    end
    for k = 1:2
        switch k
        ...
        % stLFP power
        subplot(2,3,2+3*(k-1))
        freq = LFPinfo.session(row(1)).results.cond(1).stlfp.freq;
    ...
    set(h, 'Name', 'stLFP', 'NumberTitle','off')
    ...
    for i = 1:lenr
        for k = 1:2
            pow = LFPinfo.session(row(i)).results.cond(k).stlfp.pow;
            freq = LFPinfo.session(row(i)).results.cond(k).stlfp.freq;
    ...
    set(h, 'Name', 'stLFP power bands', 'NumberTitle','off')
    ...
    stm = [];
    for i = 1:lenr
        stmidx = LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals < 1000;
        stm = [stm, LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals(stmidx)];
        for k = 1:2       
            for b = 1:5
                switch b
                    case 1
                ...
                para.cond(k).lfpstm.power.(band)(i) = ...
                    mean(LFPinfo.session(row(i)).results.cond(k).lfpstm.lfp_stm_wave(b).power(stmidx));
            end
        end
    end
    ...
    for i = 1:lenr
        stmidx = LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals < 1000;
        idx = ismember(LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals(stmidx), stm);
        for b = 1:5
            switch b
                case 1
                    band = 'delta';
            ...
            pow = LFPinfo.session(row(i)).results.cond(1).lfpstm.lfp_stm_wave(b).power(stmidx);
            para.cond(1).lfpstm.power_tuning.(band)(i,:) = pow(idx);
            pow = LFPinfo.session(row(i)).results.cond(2).lfpstm.lfp_stm_wave(b).power(stmidx);
    ...
    for b = 1:5
        ...
        if b==1
            xlabel(LFPinfo.session(row(1)).results.cond(1).lfpstm.stm.param)
            ylabel('normalized power')
        end
    end
    set(h, 'Name', 'Tuning of LFP power', 'NumberTitle','off')
    ...
    for i = 1:lenr
        ...
        for k = 1:2    
            para.cond(k).lfpstm.stlfppower.delta(i) = ...
                nanmean(LFPinfo.session(row(i)).results.cond(k).lfpstm.stlfp.band(stmidx,1), 1);
            para.cond(k).lfpstm.stlfppower.theta(i) = ...
                nanmean(LFPinfo.session(row(i)).results.cond(k).lfpstm.stlfp.band(stmidx,2), 1);
            para.cond(k).lfpstm.stlfppower.alpha(i) = ...
                nanmean(LFPinfo.session(row(i)).results.cond(k).lfpstm.stlfp.band(stmidx,3), 1);
            para.cond(k).lfpstm.stlfppower.beta(i) = ...
                nanmean(LFPinfo.session(row(i)).results.cond(k).lfpstm.stlfp.band(stmidx,4), 1);
            para.cond(k).lfpstm.stlfppower.gamma(i) = ...
                nanmean(LFPinfo.session(row(i)).results.cond(k).lfpstm.stlfp.band(stmidx,5), 1);
        end
    end
    [stm, counts] = uniquecount(stm);
    stm(counts < mean(counts)*0.3) = [];
    ...
    for i = 1:lenr
        stmidx = find(LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals < 1000);
        idx = ismember(LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals(stmidx), stm);
        idx2 = ismember(stm, LFPinfo.session(row(i)).results.cond(1).lfpstm.stm.vals(stmidx));

        for k = 1:2    
            bandtu = LFPinfo.session(row(i)).results.cond(k).lfpstm.stlfp.band(stmidx,:);
    ...
    set(h, 'Name', 'Tuning of stLFP power', 'NumberTitle','off')
    ...
    stm(counts < mean(counts)*0.3) = [];
    lenstm = length(stm);
    freq = LFPinfo.session(row(1)).results.cond(1).lfpstm.coherence.f{1};
    lenf = length(freq);
    for k = 1:2
        ...
        for k = 1:2
            for s = 1:sum(idx==1)
                para.cond(k).lfpstm.coherence_stmval(i, s) = ...
                    nanmean(LFPinfo.session(row(i)).results.cond(k).lfpstm.coherence.C{idxi(s)});      
            end
            para.cond(k).lfpstm.coherence_freq(i,:) = ...
                    LFPinfo.session(row(i)).results.cond(k).lfpstm.coherence.C{idxi(s)}';
        end
    end
    ...
    set(h, 'Name', 'spike-LFP coherence', 'NumberTitle','off')
    
%% LFP data: large ps vs small ps
elseif ps_flag==1 
    % stimulus type
    row = [];
    if strcmp(stmtype, 'rc')
    ...
    for i = 1:lenr
        ...
        subplot(1,3,1)
        wnd = 0.064;
        me = nanmean(para.cond(1).stlfp.trace, 1);
        sem = nanstd(para.cond(1).stlfp.trace, [], 1)...
            /sqrt(size(para.cond(1).stlfp.trace, 1));
        fill_between(-wnd:0.001:wnd, me - sem, me + sem, zeros(1,3))
        hold on;
        plot(-wnd:0.001:wnd, me, '-k')
        hold on;
        me = nanmean(para.cond(2).stlfp.trace, 1);
        sem = nanstd(para.cond(2).stlfp.trace, [], 1)...
            /sqrt(size(para.cond(2).stlfp.trace, 1));
        fill_between(-wnd:0.001:wnd, me - sem, me + sem, [1 0 0])
        hold on;
        plot(-wnd:0.001:wnd, me, '-r')
        hold on;
        ...
        subplot(1,3,2)
        freq = LFPinfo.session(row(1)).results.cond(1).stlfp.freq;
        me = nanmean(para.cond(1).stlfp.pow, 1);
        sem = nanstd(para.cond(1).stlfp.pow, [], 1)...
            /sqrt(size(para.cond(1).stlfp.pow, 1));
        fill_between(freq, me - sem, me + sem, zeros(1,3))
        hold on;
        plot(freq, me, '-k')
        hold on;
        me = nanmean(para.cond(2).stlfp.pow, 1);
        sem = nanstd(para.cond(2).stlfp.pow, [], 1)...
            /sqrt(size(para.cond(2).stlfp.pow, 1));
        fill_between(freq, me - sem, me + sem, [1 0 0])
        hold on;
        plot(freq, me, '-r')
        hold on;
    ...
    for b = 1:5
        ...
        for l = 1:2
            ...
            subplot(2,5,b+5)
            me = mean(para.cond(1).lfpstm.power_tuning.(band), 1);
            sem = std(para.cond(1).lfpstm.power_tuning.(band), [], 1)/...
                sqrt(size(para.cond(1).lfpstm.power_tuning.(band),1));
            errorbar(stm, me, sem, 'color', col, 'linestyle','-', 'CapSize', 0)
            hold on;
            me = mean(para.cond(2).lfpstm.power_tuning.(band), 1);
            sem = std(para.cond(2).lfpstm.power_tuning.(band), [], 1)/...
                sqrt(size(para.cond(2).lfpstm.power_tuning.(band),1));
            errorbar(stm, me, sem, 'color', col, 'linestyle','--', 'CapSize', 0)
            set(gca, 'box', 'off'); set(gca, 'TickDir', 'out'); axis square
        end
        if b==1
    ...
    for b = 1:5
        ...
        for l = 1:2
            ...
            subplot(2,5,b+5)
            me =nanmean(para.cond(1).lfpstm.stlfppower_tuning.(band), 1);
            sem = nanstd(para.cond(1).lfpstm.stlfppower_tuning.(band), [], 1)/...
                sqrt(size(para.cond(1).lfpstm.stlfppower_tuning.(band),1));
            errorbar(stm, me, sem, 'color', col, 'linestyle','-', 'CapSize', 0)
            hold on;
            me = nanmean(para.cond(2).lfpstm.stlfppower_tuning.(band), 1);
            sem = nanstd(para.cond(2).lfpstm.stlfppower_tuning.(band), [], 1)/...
                sqrt(size(para.cond(2).lfpstm.stlfppower_tuning.(band),1));
            errorbar(stm, me, sem, 'color', col, 'linestyle','--', 'CapSize', 0)
            set(gca, 'box', 'off'); set(gca, 'TickDir', 'out'); axis square
        end
        if b==1
