...
for i = 1:length(ex.Trials)
    % delta
    ex.Trials(i).lfp_delta_tc = filtfilt(filters.delta.b, filters.delta.a, ex.Trials(i).LFP_prepro);
    range = ex.Trials(i).FREQ > 0 & ex.Trials(i).FREQ <4;
    ex.Trials(i).lfp_delta_pow = mean(ex.Trials(i).POW(range));
    
    % theta
    ex.Trials(i).lfp_theta_tc = filtfilt(filters.theta.b, filters.theta.a, ex.Trials(i).LFP_prepro);
    range = ex.Trials(i).FREQ >=4 & ex.Trials(i).FREQ <=7;
    ex.Trials(i).lfp_theta_pow = mean(ex.Trials(i).POW(range));
    
    % alpha
    ex.Trials(i).lfp_alpha_tc = filtfilt(filters.alpha.b, filters.alpha.a, ex.Trials(i).LFP_prepro);
    range = ex.Trials(i).FREQ >=8 & ex.Trials(i).FREQ <=13;
    ex.Trials(i).lfp_alpha_pow = mean(ex.Trials(i).POW(range));
    
    % beta
    ex.Trials(i).lfp_beta_tc = filtfilt(filters.beta.b, filters.beta.a, ex.Trials(i).LFP_prepro);
    range = ex.Trials(i).FREQ >=14 & ex.Trials(i).FREQ <=29;
    ex.Trials(i).lfp_beta_pow = mean(ex.Trials(i).POW(range));
    
    % gamma
    ex.Trials(i).lfp_gamma_tc = filtfilt(filters.gamma.b, filters.gamma.a, ex.Trials(i).LFP_prepro);
